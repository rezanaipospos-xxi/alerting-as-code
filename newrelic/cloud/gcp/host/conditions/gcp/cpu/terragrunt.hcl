...
inputs = {
    ...
    alert_conditions_name = "GCP Host CPU High !!!"
...
*What to Do:*
```
1. Cek Penggunaan CPU Tinggi Di Server
   Command : htop
...
```
  ...
  EOF
    ...
    alert_conditions_query = <<EOF
      SELECT average(host.cpuPercent) AS 'CPU used %' FROM Metric WHERE label.hostEnvironment = 'gcp' FACET hostname
  EOF
